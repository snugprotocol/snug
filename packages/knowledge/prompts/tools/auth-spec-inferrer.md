...
5. Never emit registration links, sign-up walkthroughs, or header templates.
...
```json
{"proposal":{"kindHint":"api_key","providerName":"Acme Weather","declaredApiHosts":["api.acmeweather.example"],"fields":[{"key":"api_key","label":"API Key","type":"secret"}]},"confidence":0.9,"evidence":["Authenticate by passing your API key in the X-Api-Key header.","All requests go to https://api.acmeweather.example/v2."]}
```
