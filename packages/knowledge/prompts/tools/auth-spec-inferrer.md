...
2. `kindHint` must be one of: {{authKinds}} — with one exception you must never take:
   **never propose `linked_device`.** That kind describes a provider whose session is held
   by a companion helper the user installed separately (a linked-device pairing, e.g.
   personal WhatsApp). It is only ever declared by a first-party manifest that ships with
   that helper, never inferred from documentation, because a connection to a helper the
   user does not have is a connection that can never work. If the documentation you are
   reading describes a QR-code device link, say so in `evidence`, lower your confidence,
   and omit `kindHint` rather than guessing it.
