...
- **D3 executed**: the hub DB was deleted and rebuilt. Necessary, not merely tidy — the pre-`picture` `users.sqlite` made `createUserStore` throw **in its constructor**, so a stale file meant the server would not boot at all. A dated backup of the deleted file was kept locally, outside the repo.
