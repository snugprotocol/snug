...
- **Status**: CLOSED 2026-09-06 (owner) — **Gates 1–6 done; the branch was fast-forwarded INTO `feat/TASK-20260905-binding-a-artifacts` (the child was a linear continuation of T4's tip), so it lands with T4's PR; the AC7 re-walk is owed inside T4's AC13; this file moves to `done/` when T4 merges (ADR-0027). Follow-ups queued in next-steps: page tools for the host brain (ADR-0066 A9), the MCP-connector binding (owner direction), the A2 chat brain's cap, the recovery inferrer under Binding A.**
...
### 2026-09-06 — Jeetu (via Claude Code) — Gate 5: the diff review folded (ADR-0066 A1–A8)
- **Process slip caught by the review:** the docs commit `360e1b6` edited `prompts/README.md` without `pnpm gen:content`, so `content-drift` was red at HEAD — regenerated + committed (`92ab0ff`). Also: `/code-review 4d3b8ab` reviewed THAT commit (T4's docs-only tip), not the branch; ten finders were stopped and relaunched on `4d3b8ab..HEAD`. Lesson recorded.
- **Folded (code):** `knowledgeDeliveryFor(brain)` (`agent/knowledgeDelivery.ts`) is THE derivation for both slots and is cap-aware (a tool-less host brain declaring < ceiling + reserve gets `'none'`); `buildsToolFree` moved there; three user templates (tool/inline/unaided) with a throwing, CRLF-safe parser; `buildAppTurnContext({ toolFree })` — the EDIT turn's context block had been telling the tool-free brain to "write the file via the artifact write tool" (pre-existing, inside AC2's scope, found by the reviewer's scratch vitest); the assembly refuses `artifacts: true` with a tool-free delivery and switches with a `never` default; `renderedStore()` + `platform-truth` walk 35/36; AC2 checks the PROSE forms too (40-persistence's "schema-apply tool" reworded); 35 names the absent sections and forbids `useConnectedFetch` outright on this host (header records the Binding-A assumption); 36 says "no `fetch`/XHR"; the suffix heading is brain-neutral; refusal copy names the whole turn; T4 budget fixtures ruler-derived (self-calibrating history case); `HOST_BUILDER_RESERVED_MIN_BYTES` pinned beside the ceiling, host test imports `DEFAULT_MAX_PROMPT_BYTES`; e2e strip anchored on the hook's banner, throws on a miss.
- **Recorded, not fixed (owner decisions / next-steps):** **A8 — no shipped starter can be EDITED under the host brain** (room beside ~41.9 KB of system text ≈ 23.3 KB; smallest starter 30,176 B; S11's chess edit no longer fits). Candidate remedy: drop `20-html-template` on turns whose context block carries the app's html (turn-shape rule, ~15.6 KB back). The approved plan said "static, template whole" — this is the one point that measured differently than planned; put to the owner with a recommendation. Also: the A2 chat brain takes `'inline'` unbudgeted (cap unmeasured, not a production path yet); the recovery inferrer under Binding A (pipeline-side skip).
...
### 2026-09-06 — Jeetu (owner) — A8 decided; the way out is page tools (A9)
- **Owner decision:** the template rides WHOLE on every tool-free turn, edit turns included — mandatory. The turn-shape remedy is rejected; ADR-0066 A8 updated.
- **Owner's question:** run an MCP server (`mcp.snugprotocol.org`) so the builder fetches KB on demand across turns? **Answer recorded as ADR-0066 A9:** not an MCP server (page→connector, never model→server; viewer must connect it; bars public sharing; KB drift) — but the SAME idea one layer closer: `sample` supports PAGE TOOLS, S11 measured `limits().tools = {maxCount: 16}` on the real view, and the KB already ships inside the page. The host brain can be a tooled brain (`knowledge: 'tool'`, byok tool set as `SampleTool[]`, `artifact_edit` for edits). Queued as its own task in next-steps with the measurements it owes. The inline delivery from this task stays as the floor (no `limits().tools`, or `window.claude.complete`).
- State: unchanged otherwise (branch tip below); the AC7 re-walk is still owed and still valid (a fresh build fits).

### 2026-09-06 — Jeetu (owner) — task CLOSED; owner direction on connections recorded
- Owner direction: connected apps inside artifacts will come through an **MCP-connector binding on the page** — recorded in next-steps as its own High task (design questions + the spike it owes) and as a pointer under ADR-0065's amendments; ordered after the page-tools task.
- Closed: the branch `fix/TASK-20260906-tool-free-kb-inlining` is fast-forwarded into `feat/TASK-20260905-binding-a-artifacts` so T4 continues WITH the fix; neither branch is pushed. Nothing published.
- Next step (T4's, in a fresh session): `/pickup TASK-20260905-binding-a-artifacts` → the AC7 re-walk (rebuild, republish to the SAME artifact, build an app, journal the version id) → the remaining AC13 legs → Gate 6 + PR carrying both tasks.
