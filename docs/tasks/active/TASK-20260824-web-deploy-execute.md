...
### 2026-08-25 — Jeetu — session (🔑 SECOND PRODUCTION DEPLOY — the spacing fix is LIVE)

PR **#132** merged to `main` as **`5ccab26`** (squash, branch deleted). Deployed the website from that merged commit on the owner's ask.

- **🔑 `node scripts/deploy-web.mjs website --deploy` — `DEPLOY_EXIT=0`**, from clean `main@5ccab26 == origin/main`. **UTC 2026-08-25T00:25:00Z → 00:25:16Z.** `✨ Uploaded 6 files (77 already uploaded)`. Deployment **`c2e68afd-f573-4133-8481-9a2807a20502`** → `https://c2e68afd.snug-website-c7z.pages.dev`.
- **The owner-reported defect is GONE from the live site.** `grep -c "in<strong"` on `https://snugprotocol.org/` → **0**, and the shape scan for ANY word joined to an inline tag (`[A-Za-z,.)]<(strong|em|code|a) `) → **no matches**. Visible text now reads "…chats — **in one portable file you own**", "lives **in one portable .snug file**", "JSON Schemas**. Read the wire protocol**".
- **Verify walk, all green:** both hosts 200 · `cdn-cgi` 0/0 · `/docs/spec/`, whitepaper PDF, `/docs/`, `/download/` 200 · playground `/settings` 200 · local-mode tripwire 0/0.
- **LANDMINE FOUND AND CLEARED — `CLOUDFLARE_API_TOKEN` in root `.env` BREAKS the deploy script.** The first dry run after merge **refused**: `REFUSED: pnpm exec wrangler whoami exited 1`. Cause: **wrangler reads the root `.env` itself** (as `.env.example` warns), and a `CLOUDFLARE_API_TOKEN` there takes precedence over the OAuth session; that token lacks the `user (read)` scope `whoami` needs, so account enumeration fails and the script's pre-flight correctly refuses. Unsetting the shell variable was NOT enough — the value comes from the FILE. **Removed the token line from `.env`; `whoami` immediately recovered** (`jeetu@techvoyage.org`, `e930cbfd…`) and the deploy proceeded. This vindicates ADR-0054's "never put a token here" as more than a secrets rule: **the token also breaks the tool**. The token value is preserved outside the repo at `…/scratchpad/env.backup` for whoever finishes AC6/AC7 — put it in the ENVIRONMENT for those API calls, never back into `.env`.
- **Task stays in `active/` (ADR-0027): AC6 and AC7 are still open**, so this is not done. **AC1–AC5, AC8–AC12 MET.**
- **Next step (owner):** add Zone → **Config Rules: Edit** and Account → **Account Rulesets: Edit** to token `921ab1541c5037d757319885ddba68fc`, export it in the shell (not `.env`), then create the `www` → apex 301 rule and the `snug-pages-dev` Bulk Redirect (sources **`snug-website-c7z.pages.dev`** + `snug-playground.pages.dev`; subpath ON, preserve path/query ON, include-subdomains OFF) → re-verify → **revoke the token**.
- **Still owed beyond AC6/AC7:** the by-hand browser walk (teaser plays, playground boots demo-mode, `about ↗` lands on the apex) and the deferred preview Access policy (**`--preview` must not be run until it exists**).
