...
- Verification performed: `gh release list` shows the helper as Pre-release and desktop v0.1.1 still **Latest**; `releases/latest/download/latest.json` → `0.1.1`; the aarch64 asset 302s to `release-assets.githubusercontent.com` (in the shell's host allowlist — this CDN host was not in the plan's original list and was added on the review's prompting); downloaded bytes sha256 = the pin; `.sig` downloads; the staged archives verify against the PRODUCTION updater key from Rust.
- `check-public-scrub` run by hand before the release — OK.

### 2026-08-26 — Claude — review (Gate 4) + self-sign-off
- `/code-review high` (8 finders): 40 findings; all correctness ones fixed in-branch — status/swap race (status is now pure; the `.old-*` restore runs only under the sidecar lock), restart of the old helper when a swap fails, chip surviving a failed install, `.download-*` litter reaped, per-read timeout instead of a 600 s whole-request deadline, `.sig` fetched before the 42 MB archive, sha256 during the stream + streaming minisign (no 42 MB heap copy), verify/unpack/swap off the async runtime thread, `broken` classification when a downloaded tree loses `bin/node` (status and spawn agree), union semantics of fetched+bundled notes restored, the private key never on argv, pin moved to `helpers.json` (one file, zero regex parsers; release-desktop now byte-compares the published manifest), `ConfirmOverlay` as the one portaled primitive, dev install builds beside the target and swaps. Deferred with a reason: converting the five other pre-existing `.net-confirm-overlay` sites to `ConfirmOverlay` (none is header-mounted; separate cleanup), a typed refusal from `sidecar_ctl('start')` itself (net.ts runtime moment is covered by the run-surface card), pack-script I/O tuning (owner-only release path), an uninstall surface (residual R-33).
- **Self-sign-off (High tier, PROCESS.md):** plan had its fresh-context AI review before code; TDD throughout with negative tests (bad sig, wrong pin, traversal/symlink/bomb archives, off-host redirects, dev-tree refusal); gates green (root `pnpm test`, cargo 121, playground 1694, desktop 188, auth 939, sidecar 177); the release was explicit-ask and is journaled with verification. Signed off by Claude on behalf of the session; owner walks AC14 remain owed (next-steps).
