...
### 2026-08-10 — Claude — P0 implemented; three-lens adversarial review returned REVISE; two BLOCKERs re-verified BY EXECUTION

- Implementation ran as a four-stage workflow (design → red → green → review): 11 agents, 0 errors. All three package suites green at the end of Green: **protocol 221 · auth 222 · db 232**, run live.
- **The additive cutover rule (B1) held, verified directly rather than from agent reports:** `llmProposalSchema` still exported (`packages/protocol/src/index.ts:144`), `snug_auth_specs` still in `USERDB_TABLES` (`:68`) alongside the new `connections: 'snug_connections'` (`:69`), `USERDB_SCHEMA_VERSION = 4` (`:36`). Main's shipped install-act surface is intact.
- **Helper trim (fold S-M2a) landed:** `unix_ms`, `hmac_sha512`, `sha256` deleted. Net 6 → 4.
- **The encoding decision (fold F-m3) went to `add_helper_variant`, with better reasoning than the plan specified.** Rather than a general `base64decode()` primitive, the design FUSES decode+HMAC+base64 into one fixed-shape helper `hmac_sha256_b64` — so the decode can only ever be aimed at the key argument and only ever feeds HMAC, never arbitrary text. The design agent also declared its confidence honestly (HIGH on Exchange's scheme, LOW that a hex-expressible Advanced Trade variant is still current) and rejected option (b) on the grounds that pinning to Advanced Trade would delete the *passphrase* — the third field whose absence is the defect the owner filed. That is the right call: it refuses to resolve a decision by redefining the requirement.
- **Three-lens fresh-context adversarial review: REVISE from ALL THREE lenses** — 3 BLOCKER (2 distinct, one found independently by two lenses), 5 MAJOR, 4 MINOR. **The bar earns its keep a seventh consecutive time, and this is precisely the gate the autonomous run would otherwise have skipped.**
- **The two distinct BLOCKERs were re-verified by the orchestrator EXECUTING them, not by reading the reports:**
  - **BLOCKER-1 — quoted-argument lint/engine parity break is a live credential-exfiltration hole.** Probe: `lintAuthHeaderTemplate({X: "{{base64('api_key')}}"}, {fieldKeys:['api_key']})` → **`ok: true`**; `renderAuthHeaderTemplate(same, {fields:{api_key:'SUPERSECRET'}})` → **`{"X":"U1VQRVJTRUNSRVQ="}`**, which base64-decodes to `SUPERSECRET`. Root cause: the lint skips quoted args as "literal by authorial intent" (`template-lint.ts:203–205`) while the engine's `parseHelperArgs` strips the quotes WITHOUT recording quotedness (`template-engine.ts:229–236`) and passes the dequoted text to `resolveArgToken`, which resolves it as a field. So the lint's stated parity promise (`template-lint.ts:118–124`, "a template cannot lint one way and render another") is false — for exactly the base64 primitive the threat model already flags as scrub-evading.
  - **BLOCKER-2 — the pinned Coinbase-Exchange template is STILL inexpressible, so the fourth helper's justification does not hold.** Probe: the full pinned template `{{hmac_sha256_b64(api_secret, timestamp(), request.method, request.pathAndQuery, request.body)}}` → **lint `ok: false`**; even the minimal `{{hmac_sha256(api_secret, timestamp())}}` → **`ok: false`**. `timestamp()` is not an accepted argument form in either the lint or the engine (no nested-helper parsing in `splitHelperArgs`/`resolveArgToken`). Consequence: ADR-0017's central justification for widening a deliberately-minimal enum is unrealized, AND the `RenderState` timestamp memoization — pinned in the ADR as fixing a real ~1-in-N intermittent auth failure — currently protects nothing reachable.
- **Other findings to fold:** registry-borrow ban substitutes hosts/name/endpoints but leaves attacker-authored `fields` labels, `request.headerTemplate` and `testRequest` intact (credential MISDIRECTION — the exact harm `llmProposalSchema`'s omit-list existed to prevent); `admitConnectionRequirement` has **no production caller**, so AC5's userLayer channel guard is unreachable and its tests pass while the property does not hold at the only path that persists a requirement; `approveConnection` neither clears nor refuses a staged pending requirement (leaves a just-approved row reading "needs re-approval" and parks an unreviewed requirement for the next promotion); `putDeclaredConnection` never checks `slot === requirement.slot`; AC7 is a tautology (compares the lint's constant to a retyped copy, never reads the engine's map); AC8's grep-proof is co-occurrence, not ordering.
- **Honest note on the two carve-out deliverables:** ADR-0017 was written (22KB), but the **v0.3 spec draft and the INTERNAL-DRAFT spec-changelog entry were NOT produced** by the docs stage. Those are the two items the owner's carve-out approval unblocked, and C3 + Gate 6 require the changelog entry. They are outstanding and land in the fix round.
- NEXT: fold round — fix the two BLOCKERs and the MAJORs test-first, write the two missing docs, then re-review before P0 closes.

### 2026-08-10 — Claude — P0 review FOLDED; both BLOCKERs closed and re-verified by execution; P0 COMPLETE

- Fold ran as a two-stage workflow (fix → independent re-verify), 5 agents, 0 errors. **Both re-verifiers returned APPROVE with `blockersRemaining: 0`.** As with the review, the orchestrator re-ran the original probes rather than trusting the fix reports.
- **BLOCKER-1 (quoted-arg credential exfiltration) — FIXED IN THE ENGINE, which is the direction that removes the capability rather than merely narrowing what reaches it.** `parseHelperArgs` now carries per-argument quotedness (mirroring the lint's `splitHelperArgs`), and `resolveArgToken` returns quoted arguments VERBATIM as its first branch — before the `request.*` check and before the `ctx.fields` lookup. **Orchestrator's own probe, re-run against the fixed build: `{{base64('api_key')}}` now renders `YXBpX2tleQ==` → decodes to the literal `api_key`** (was `U1VQRVJTRUNSRVQ=` → `SUPERSECRET`). The lint's parity claim is now true of shipped code. The complement is pinned too: UNQUOTED `{{base64(api_key)}}` still resolves the field, so this is not blanket literalization. A re-verifier went beyond the handed case with 18 adversarial shapes plus a 30-permutation fuzz (adjacent-quote splicing, escapes, mixed/unbalanced quoting) — 0 leaks; and proved the probe is a live detector by mutating the fix out and watching 15 leaks reappear.
- **BLOCKER-2 (Coinbase inexpressible) — FIXED VIA OPTION (b): a fifth pinned request token `request.timestamp`.** Chosen over nested-helper parsing because a token is enforced by the token list itself, whereas "zero-arg only, one level deep" would be a rule in code that a later edit can relax; nesting stays REJECTED by both lint and engine, so the flat grammar cannot creep. Both `{{timestamp()}}` and `{{request.timestamp}}` route through a single `renderTimestamp(state)` seat, so the two spellings cannot disagree by construction rather than by two call sites agreeing.
  **Orchestrator's own end-to-end verification** (`scratchpad/p0probe2.mjs`): the four-header Exchange template lints `ok:true`, renders, and an INDEPENDENT recompute using node's `createHmac` — a different implementation from the engine's WebCrypto — of `base64(HMAC-SHA256(base64decode(secret), sentTs+method+path+body))` **matches the engine's `CB-ACCESS-SIGN` byte-for-byte**, built from the timestamp AS SENT. The credential does not appear in the output in plaintext. **The motivating defect of the entire rewrite is now expressible and correct end-to-end.** A re-verifier additionally spun 77,314 renders across ~2.5s with 0 signed/sent mismatches, and caught the memoization mutation with a clock advancing a second on every read.
- **MAJORs closed:** the registry-borrow ban now also clears the credential-prompt seats (`fields`, `request.headerTemplate`, `testRequest`) on a borrow hit, so a hostile requirement can no longer wear a registry provider's name while carrying attacker-authored field labels; admission is now ON the persist path; `approveConnection` handles a staged pending requirement; `slot === requirement.slot` is enforced on all three write paths. AC7 is no longer a tautology (the ENGINE's exported helper-name set is asserted against the lint enum, mutation-evidenced with a dummy helper); AC8 was closed by NARROWING THE EXPORT SURFACE — the unlinted `renderAuthTemplateString` is no longer re-exported, which is a structural fix rather than a better grep.
- **The two owner-approved carve-out deliverables landed**: `docs/spec-drafts/spec-v0.3-auth.md` (26KB) and the INTERNAL-DRAFT `docs/spec-changelog.md` entry, explicitly marked "not staged for any push". **SPEC_SYNC steps 1–3 + 6 taken; steps 4–5 NOT taken. Nothing was pushed to `snugprotocol/spec`; AL-12 remains HELD.**
- **Test counts, all re-run live by the orchestrator (not asserted from agent reports): root `pnpm test` 19/19 packages green · protocol 221 · auth 240 · db 243 · playground 482.**
  **A count worth explaining rather than hiding:** playground reads 482 here against the 512 recorded in the parent's baseline. That is NOT a regression — 512 was measured on the AL-09 branch, which carries five extra starter apps and their tests. Verified by checking out `main` and running the playground suite there: **main is also 482.** The parent plan's baseline line should be read as branch-specific.
- **One PRE-EXISTING security finding surfaced, confirmed by the orchestrator, and deliberately NOT fixed in P0 — routed to P5:** the registry-borrow ban is evadable by a brand-ADJACENT provider name. `normalizeProviderKey` (`well-known-providers.ts:149`) is `toLowerCase().replace(/[^a-z0-9]/g,'')`, which collapses case and punctuation but NOT added words. Probe: `"Spotify"`/`"SPOTIFY!"`/`"Spotify "` all hit the registry (ban fires), but **`"Spotify Inc"`, `"Spotify Connect"` and `"Spotify-Premium"` all MISS it and are admitted with attacker-authored fields + header template**. Confirmed pre-existing: `git diff HEAD -- packages/auth/src/well-known-providers.ts` is empty. P0's new host-intersection ban catches this ONLY if the attacker also declares a registry host; a brand-adjacent name on the attacker's own host still passes. This is the same family as the ASCII-lookalike limit ADR-0017 already scopes honestly, and it belongs with P5's threat-model delta rather than being quietly patched here.
- **P0 EXIT MET:** v4 contracts green ALONGSIDE v3 (cutover rule verified — `llmProposalSchema` still exported at `index.ts:144`, `snug_auth_specs` still present); write rules, template lint and slot cap mutation-evidenced; self-heal + legacy-slice fixtures proven; `importUserDb` reconciliation green; draft + changelog entries exist; no UI. NEXT: **P1 — runtime.**
