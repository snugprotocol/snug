...
- Done: re-verified the branch from a clean checkout before opening the PR — `pnpm --filter examples test` **261/261**, `pnpm --filter playground test` **1244/1244** (121 files, tsc-gated). Both match the Gate-5 numbers, so the fix round is confirmed stable. Pushed `feat/TASK-20260819-starter-sample-data` and opened **PR #76**; the PR body records the render-only plan deviation, all five Gate-5 fixes, and the accepted residuals.
- State: PR #76 open against `main`, all local evidence green. CI is billing-blocked (next-steps 2026-08-19), so the red X's on the PR page are the absent gate, not these suites.
- Next step: owner walk of the four sample surfaces on desktop (connected starters are desktop-only/env-gated — no test covers the visual), plus the 3 pending Telepath linking-sync walk items re-run on a build containing this change.
- Open questions: none blocking. Reminder carried forward: already-installed starters (incl. the owner's linked Telepath) will report `html_mismatch` against the new factory bytes and that surface still has no renderer — accepted 2026-08-19, sample mode reaches new installs only.
