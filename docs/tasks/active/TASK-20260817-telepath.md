...
4. **Live updates**: a message arriving while the app is open appears in the open thread, re-sorts
   the chat list, and bumps the badge without user action. (sidecar `/events` cursor + long-poll
   tests; host pump unit tests incl. stop-on-unmount and backoff; app-core event-merge reducer tests)
...
6. **Analysis lifecycle**: first analyze fetches full history (paginated), records the
   max-message-ts watermark in the app DB, stores the full analysis run; a subsequent analyze builds
   its request from the stored prior analysis + only messages after the watermark. (request-builder
   tests prove full-vs-delta payload composition; DB layer tests prove watermark/run persistence)
...
9. **Docs ingestion**: installing the starter seeds `snug_app_docs` with
   ...
   prompt; existing user docs are never clobbered on re-install. (playground install tests; AC9
   provenance gate still green)
10. **C1/C2 negatives (High tier)**: new routes refuse traversal (TS + Rust twins, decoded-form);
    `/pair/*`+`/session/*` still refused to apps with the widened table; pump-emitted host-events
    carry no credential material (negative scan test); media/base64 responses enforce the 1 MiB cap
    while reading (Rust) and the sidecar refuses oversized media with a structured
    `{tooLarge:true}` rather than truncating.
...
**Design decisions already made (owner may veto at approval):**
- **D1 — Push rides the existing `snug:host-event` channel** (R2: open, additive, unknown events
  ignored). No new protocol frame, no C3 spec-sync on `schemas/`. The "true push seam" =
  sidecar event buffer + long-poll route + a host-side pump that forwards into the running app's
  frame via `RunnerHost.notifyEvent('connection-event', {slot, events})`.
- **D2 — Long-poll, hold ≤ 8 s** (server's `REQUEST_TIMEOUT_MS` is 10 s; verify the Rust unix-socket
  read has headroom, else shorten). Pump re-polls immediately; app-perceived latency ≈ instant.
...
- **D4 — Media as base64 JSON** through the existing 1 MiB-capped transport: inline `jpegThumbnail`
  rides with each image message; full image via `GET /chats/:jid/media/:id` only when raw bytes
  ≤ ~700 KB (base64 headroom under the cap), else `{tooLarge:true}` + thumbnail. Avatars via
...
- **D5 — Unread counts are Baileys-reported**, incremented by live events, cleared locally on
  thread-open. No read receipts sent.
...
- **D8 — Doc ingestion is generic** (any starter shipping `authoring/docs` gets seeded at install),
  implemented as a sibling of `installStarterConnections`/`installStarterRuntimeContract` with its
  own explicit `?raw` glob; the validate-suite pin (app-html glob can't reach `authoring/`) stays,
  extended to shape-check the new glob. ADR-0035 records the deliberate reversal of
  "provenance never ships".
...
- **B — Sidecar** (`apps/whatsapp-sidecar`): extend `WaMessage` (`kind: 'text'|'image'|'unsupported'`,
  ...
  `downloadMediaMessage` on demand with the size refusal), `profilePictureUrl` fetch, unread/chat
  metadata from Baileys events, bounded event ring buffer with a monotonic cursor (drop-oldest +
  `resync:true` flag when a cursor has aged out). `router.ts`: three handlers incl. the ≤8 s hold.
  Tests extend `router.test.ts`/`fake-wa-socket.ts`/`message-mapping.test.ts`: cursor semantics,
  hold-until-event, empty-timeout shape, media cap refusal, auth still required on every new route.
...
- **D — Host live pump** (`apps/playground`): new `src/state/sidecarLive.ts` — start/stop keyed to
  ...
  cursor state; exponential backoff on failure; forwards batches via `notifyEvent`. Wire in
  `RunView.tsx` beside `controlsRef`. Tests: fake executor → events reach `notifyEvent`;
  stop-on-unmount kills the loop; no credential material in any emitted payload (C1 negative);
  backoff verified with fake timers.
- **E — The app** (`examples/whatsapp` rebuild): marker-block core v2 (pure, extracted+tested by the
  ...
  (new), event-merge reducer + message dedupe-by-id (new). UI: iOS-style list/thread/tabs
...
- **F — Doc ingestion** (`apps/playground` + `examples` validate): `installStarterDocs` sibling with
  its own `authoring/{docs,prompts}/*.md` glob; seeds absent slugs only; `build-prompt` slug from
  `prompts/01-build.md`. Wire into the explicit starter-install path (NOT the hub's open-only tiles).
  Tests: seeding, no-clobber on reinstall, glob-shape pin extension. ADR-0035 accepted here.
...
- Done: explored POC + platform (two fresh-context sweeps), owner interview (4 answers), task file,
  draft ADR-0034/0035, branch created.
- State: plan written; fresh-context AI plan review to run before owner approval (High tier).
- Next step: fold review findings → owner approval → Phase A tests.
- Open questions: none blocking (D1–D8 vetoable at approval).
