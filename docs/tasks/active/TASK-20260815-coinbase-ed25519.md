...
2. Honest typed errors, never silence: a legacy EC key (SEC1 `BEGIN EC PRIVATE KEY` or
   PKCS#8 with id-ecPublicKey OID) names the fix ("create an Ed25519 API key…"); an
   undecodable paste errors typed; a runtime without WebCrypto Ed25519 fails LOUDLY.
   Negative (C1/C5): no error string ever carries key bytes or pasted content.
...
6. Honest done screen: for a static-kind row that carries a `testRequest`, the done
   screen claims "connected" only after a passing probe; until then it says the
   credentials are SAVED and offers the test (today "X is connected" renders regardless
   of any probe outcome — the second silent path the owner hit).
...
1. Generate a deterministic Ed25519 test fixture offline (`openssl genpkey -algorithm
   ed25519`): PKCS#8 PEM + derived base64 64-byte (seed‖pub), 32-byte seed, public key
   for verification. Keep one EC SEC1 fixture for the rejection tests.
...
3. Update registry pins: `registry-request-seats.test.ts` (renamed arg in template +
   ...
   (field-key pin). `template-lint.test.ts`: enum unchanged; arg-rule fixtures updated
   only if they name the old field key.
...
7. `src/__tests__/registryDriftMigration.test.tsx`: NEW case — approved EC-era row
   ...
   `private_key` secret is deleted at re-approval. Then implement the orphan-secret
   deletion (ADR-0028 precedent `:530-540`): in the re-approve path, delete stored
   credential secrets whose keys are no longer declared fields. Existing drift suites
   (old-triple staging, seat persistence) updated for the new shape.
8. AC5: test in `src/__tests__/connectionWizard*.test.tsx` — probe path whose executor
   ...
   in `testConnection` (typed outcome `{ok:false, code:'NET_INTERNAL', message}`) plus
   a `.catch` on the `runTest` chain in `ConnectionWizardSheet.tsx`.
9. AC6: test — probeable static-kind done screen shows "saved" + test affordance, flips
   to "connected" only on probe `ok`; non-probeable kinds keep today's copy. Implement
   in `DoneScreen` (`Sheet:1137-1234`). Update `coinbaseJourney.test.ts`,
   `registrySeatPersistence.test.ts` (CDP literals), `wholeSurfaceP6.test.tsx` (labels).
...
12. Docs in-branch: ADR-0030 finalize; ADR-0022 status line gains the superseded-§2
    ...
    swallow surfacing); `code-map.md` if it names `es256-key.ts`.
...
- **WebCrypto Ed25519 availability** in CI Node and jsdom/vitest workers — probe FIRST
  in Phase A (a 3-line spike test); if a runner lacks it, tests pin the honest-error
  path there and the signing tests gate on runtime support.
