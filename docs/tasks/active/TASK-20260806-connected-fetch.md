...
### 2026-08-06 — Claude (Fable 5, final-gate sweeper) — inspector lead RESOLVED + full live sweep re-run: CLEAN, no code change

- **The interrupted sweep's inspector lead is resolved, two halves, neither a blocker.** (1) *"NO entries at all are showing"* = **harness artifact**: the interrupted session left a vite dev server running since ~2026-08-03 (2d10h old — predating every AL-03 commit and the rebase) plus a stale agent-browser daemon and net stub; on a FRESH dev server the frames timeline populates immediately (ready/announce on boot, live-verified). (2) *"`summarize` has no case for net frames"* = **true but non-blocking**: net frames DO route to the inspector (runner `onFrame` fires at host.ts inbound line for `net-request`, and `net-response` goes out via `post()` → `onFrame('outbound')`; RunView dispatches every frame unfiltered), they just render as generic "frame / (empty)" entries; the `ready` summary also omits the `net` capability bit. NO AC or design decision (D1–D6, B1–B3, R1–R5, A1–A5) promises net-specific inspector summaries, and the structural-only rule (AC-4 of the think-panel task) means the generic entry leaks nothing — C1-probed live: no injected value anywhere in the rail. Logged as a dated next-steps row pointing AL-05 (teach the timeline) and AL-11 (state value-blindness as a boundary).
- **Full live sweep re-run on a fresh vite dev server (port 5175, clean OPFS origin) + fresh self-signed https stub, Chromium with `--host-resolver-rules=MAP stub.snug.test 127.0.0.1` scoped to the sweep browser. All seven items PASS:**
  1. **Connections panel** — seeded spec (the only creation path pre-AL-04 is app/db-declared, by design) renders provider/kind/status pill/full host list; approve → "approved" + re-approve/revoke swap; revoke → empty-state copy returns. Dev-grade visuals per D5. (Earlier "click did nothing" was the button below the fold — real clicks work once scrolled into view.)
  2. **Injected fetch** — approved GET via a hand-rolled bridge app: `ok:200`, body renders `"sawApiKey":"***"` (stub demonstrably RECEIVED the key — 401 otherwise — and the scrubber redacted it), etag renders `W/"***"` (R1 in a whitelisted header), `x-ratelimit-remaining` crosses, `set-cookie` and `x-powered-by` never appear (A2 + whitelist).
  3. **Confirm gate** — POST raises the modal (names app, method, host; "credentials attached by the host — the app never sees them"); deny → `NET_CONFIRM_DENIED` with ZERO network activity (request count unchanged); allow → 200; remember-checkbox → second POST sails through with no dialog.
  4. **Grant invalidation** — R3 live: after settings re-approve, the remembered grant is dropped and the next POST prompts again; page reload also drops grants (in-memory only). Revoke → next fetch fails closed `NET_NOT_APPROVED` ("no approved connection exists for this app"), executor reads the spec per-use.
  5. **C1 probes** — the secret literal is ABSENT from: full a11y tree incl. iframe text, host DOM outerHTML, console, localStorage/sessionStorage, the whole think-rail text, network request URLs, and the DEFAULT `.sqlite` export bytes (`includeSecrets:false` export byte-grepped clean; opt-in export contains it — designed user-custody behavior). Hosts contacted the entire session: dev server, the stub, api.anthropic.com (byok by design), cdn.jsdelivr.net (C2 allowlist) — nothing else.
  6. **Real byok build** — key set page-side from `internal/.env.local` (never echoed into logs/files), provider anthropic: 5 real round trips (claude-sonnet-5, token counts + per-turn cache % rendering in the LLM inspector), "Hello Button" app built and runs; byok key absent from DOM/localStorage; net-sweep GET still `ok:200` scrubbed AFTERWARD in the same session (coexistence). Key deleted from the test profile db after the sweep.
  7. **Hygiene** — zero page errors, zero CSP violations, console carries only pre-existing dev noise (vite debug, React Router future-flag warnings). Live C2 spot-check: `sandbox="allow-scripts"`, `connect-src 'none'`, fixed CDN allowlist. Known pre-existing StrictMode `?idea=` handoff cancel reproduced on the hub→build path (issue #13, not this branch).
- **One new non-blocker found and logged for AL-04:** revoke (`deleteAuthSpec`) leaves the `auth:<appId>:*` secret slice in place — unreachable (fail-closed proven) but re-approval of a same-id spec would resume injecting the old value; wizard should decide revoke-vs-disconnect (slice-wipe exists in CredentialStore).
- **No code changed; docs-only commit** (this journal + two next-steps rows). Sanity after commit: auth + playground suites re-run green (see commit). Gate verdict: **MERGE READY** — PR/merge remains the orchestrator's step per the run rules.
