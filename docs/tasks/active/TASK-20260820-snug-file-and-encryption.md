...
- **Status**: in-review — **PR #89** (https://github.com/snugprotocol/snug/pull/89), OPEN and mergeable, not yet merged. Both mandatory reviews done, all findings fixed, owner-found wiring gap fixed, full root suite green.
- **Owner**: Jeetu
...
### 2026-08-20 — Jeetu — close-session (Gate 6)

- **Done this session, end to end**: Gates 1–5 for both halves of the task. `.snug` rename with read-and-adopt-forward for the file, its sync sidecar and the Dropbox path; the `SNUGENC1` container with key-wrapping, two independent unlock slots and a mandatory ≥128-bit Recovery Key; the encrypted persistence/export/personal-origin-sync boundary (hub origins deliberately unchanged); the `'locked'` boot state and `unlockUserDb`; the Hub offer, setup flow, unlock screen and Settings controls; ADR-0042 + ADR-0043 + ADR-0027 status lines on 0007/0009/0014/0021; threat-model v2 (R-3 rewritten, A6 defended, R-14 added, §7 claim scoped, delta ledger re-pinned); spec v0.2-userdb §5+§6 staged with a changelog entry; whitepaper + figures + README/glossary/architecture/code-map/examples + KB regen.
- **Both mandatory High-tier reviews ran and both paid.** Plan review: 9 blockers before any code, three of them data-loss (sidecar orphaning → phantom divergence whose obvious resolution discards local work; Dropbox split-brain; second-device unlock impossible as designed). One finding was **tested and refuted** — a KDF-parameter flip breaks the unwrap, so it is DoS not silent weakening; AAD was adopted anyway for the honest reason. Diff review: **no defect in the container** under direct attack (fuzzing, forged slot counts, truncation sweeps, 50-reseal nonce check), but four wiring defects including **D-2, where the composition root never passed the sealer** — so ADR-0043's central promise did not hold in the shipping app.
- **Self-caught**: the Recovery Key was 127.6 bits against my own 128-bit floor (30-glyph alphabet sized as base-32), with a test that hardcoded `log2(32)` and hid it. Now 27 symbols = 132.5 bits, entropy derived from the observed alphabet.
- **Owner-found, and the most important outcome of the session**: the feature was **unreachable** — `ProtectSetupFlow` never rendered, `initProtectOffer` never called, 1341 tests green over a screen no user could get to. Then the same class recurred one layer down (a wrong passphrase showed nothing, because `unlockUserDb` published a transient status that unmounted the screen mid-attempt). Both fixed and then **verified by driving the real app**, not by tests.
- **Exact state**: working tree clean; branch `feat/TASK-20260820-snug-file-and-encryption` fully pushed, 0 unpushed commits; **PR #89 OPEN and MERGEABLE, not merged**; full root suite **23/23 tasks, exit 0** (playground 1348 · auth 915 · db 391 · protocol 344 · knowledge 184 · sidecar 152 · desktop 140 · adapters 130 · server 126 · runner 119 · sdk 41; threat-model 139/139; whitepaper 70/70). `gate:local` **4/6 legs green** across two runs (workspace, smoke, rust, desktop); `e2e` and `release` **not run and not claimed**.
- **Single next step**: run `pnpm gate:local --legs=e2e` (the lane owning the export locator this task renamed, ~10–20 min), then owner review and merge on local evidence per the standing CI-billing condition.
- **Open questions**: (1) run the e2e leg before merge, or merge knowingly without it? (2) when to push spec v0.2-userdb §5+§6 to `snugprotocol/spec` — staged, needs an explicit ask. (3) a desktop-shell manual walk is owed; this session's walk was the web path.

### 2026-08-20 — Jeetu — spec push (SPEC_SYNC step 5)

- **Done**: owner gave the explicit ask; pushed `snugprotocol/spec@8ea69b8` to `main` as a single commit (one file, +118/-3). The staged sections land as **§6 and §7** — the published `SPEC-v0.2-draft.md` already used §1–§5, so the numbering shifted on publication. Also amended in the same commit: §2's canonical filename, §2.2's per-app export, §3.2's hub export obligation, and a dated revision note. Every offset in §7.1 was checked against `container.ts` before pushing (checksum at 34, header 38, slot 13 B, wrapped 48, salt 16, IV 12) — the spec must not describe something the code does not do.
- **Deliberately NOT pushed** (owner decision): `schemas/host-ready.json` drift. The spec repo's copy lacks `net`/`openUrl`, but that originates in EARLIER tasks — publishing it here would have broken the one-commit-per-task invariant and consumed another task's still-pending ask. Recorded in `spec-changelog` and `next-steps` rather than silently carried.
- **Also found and recorded**: the local spec clone had an **unpushed** `docs/whitepaper-v0.1` branch (`ae6ec8b`) that has never reached `origin/main`. Untouched — it is not this task's to decide. Now in `next-steps`.
- **State**: `snugprotocol/spec` main is at `8ea69b8`; changelog carries the SHA; SPEC_SYNC steps 1–6 complete for this task.
- **Next step**: unchanged — run `pnpm gate:local --legs=e2e`, then owner review and merge of PR #89.
