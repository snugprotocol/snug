...
- **Related**: [threat-model.md](../threat-model.md) — **raised v2.0 → v3.0 by this task** · [`docs/security/`](../security/) (12 deltas) · ADR-0040/0045/0046/0047 · `internal/RUNBOOK-flip-public.md` · [lessons.md](../lessons.md) (esp. 2026-08-21 plan-review + signed-artifact entries) · [next-steps.md](../next-steps.md)
...
### 2026-08-21 (later) — Jeetu/Claude — session (Gates 3–6)
- **Owner approved the plan** with both open questions answered: (1) take the `sidecar_wizard_fetch` fix in-scope; (2) **no ADR** for the launch posture. Authorised to run all phases without further approval, explicitly excluding the merge.
- **Done — Phase 0 remainder:** `gitleaks` installed (8.30.1, Homebrew) so AC6 stage 1.5 could actually execute rather than being reported as skipped; `internal/scrub-terms.txt` authored from `.env.local` paths + segments + codenames (18 terms, later pruned to 17 — `feat` alone matched 322 lines, and a gate returning hundreds of false positives is a gate nobody reads).
- **Done — Phase 1:** seven read-only lanes dispatched in parallel against committed tree `002984a`, each briefed with the mechanism-claim rule, default-to-refuted discipline, and the known-stale figures (R-12) so no lane could "confirm" a number by reading the document asserting it.
- **Done — Phases 2–4:** eight findings actioned (table above), each re-verified against primary sources before acceptance. Four security/hygiene fixes landed test-first with the RED recorded and each mutation-checked. Threat model rewritten to v3.0. Seven non-blocking findings filed to `docs/next-steps.md` with evidence.
- **Done — Phase 5 verification:**
  - `pnpm run gate:local --all` → **FULL PASS, 6/6 legs** (workspace · smoke · e2e · rust · desktop · release); the gate's own verdict reads "equivalent to ci.yml on macOS" and names the Windows leg as having no local counterpart.
  - `pnpm --filter desktop gate` → **GATE GREEN in a real WKWebView**, run separately after the last commit. `ipc-sidecar-wizard-fetch-refused` and `ipc-sidecar-wizard-fetch-dispatchable` both executed and passed there, so R-12's closure is proven behaviourally rather than only unit-pinned. Checked for a stale `snug-desktop` process first (2026-08-19 lesson) — none.
  - Per-package: auth 916/916 · playground 1465/1465 (serial — see below) · desktop 175/175 · knowledge 185/185 · db 418/418 · examples green · `check-threat-model` 175/175.
- **One red investigated and dismissed with evidence, not assumption:** `starterInstall.test.tsx` timed out under full parallelism, passed in isolation, and passed in a full serial run (1465/1465). That is the documented contention flake's signature — a varying failure set — and not this branch's change. No config touched, per the standing rule against patching an unreproducible symptom.
- **State:** all phases complete. 13 commits on `feat/TASK-20260821-launch-security-review`, pushed; **PR #96 open and NOT merged** (owner's explicit instruction). Working tree clean.
- **Next step:** owner reviews PR #96. On merge: move this task file to `done/` with an INDEX line. The follow-on work the owner named — regenerating the spec/whitepaper in a fresh session, then the public flip — is gated on that review, and the spec correction in this branch (SNUGENC1 §11.1) is written to ride that regeneration rather than being pushed here.
- **Open questions:** none blocking. The verdict's conditions are owner actions, not decisions: CI billing, the SSO-runbook caveat, the hardware walks, runbook stage 0, and the 36-file `internal/` reference scrub.
