...
- **Packages touched**: `apps/website` (deps + `astro.config.mjs`), `apps/playground`, `apps/desktop` (devDeps), root `package.json` (vitest, `pnpm.overrides`, `audit:deps`), `scripts/audit-deps.{mjs,test.mjs}` + `scripts/audit-allowlist.json` (new), `pnpm-lock.yaml`; NO source in `packages/*`
- **Spec impact**: none
...
### 2026-08-24 — Jeetu/Claude — session (Gates 3–5)
- **Done — all four steps complete.**
  - **Step 0 (AC5):** `scripts/audit-deps.mjs` + 17 node:test cases written FIRST (confirmed red on a missing module before implementing). Four of my own tests were over-specified and were corrected, not loosened: a malformed allowlist entry legitimately produces TWO failures (the malformation AND the advisory it failed to excuse — asserted on both), and the `tool: "cargo"` filter had to be mirrored in the self-check. Root script `audit:deps` added; deliberately NOT in root `pnpm test`.
  - **Step 1 (15 alerts):** vite 5.4.21→6.4.3 (desktop, playground), vitest 2.1.9→3.2.7 (root, website). vite 5 and esbuild 0.21.5 left the tree entirely. **Plan correction: nanoid comes from `postcss` (a vite dep), not vitest** — so the override I had planned as an optional backstop was mandatory. First attempt `">=3.3.18"` resolved to **nanoid 6.0.1** (three majors, ESM-only); pinned to `"^3.3.18"` → 3.3.18.
  - **Step 2 (19 alerts):** astro 5.18.2→7.2.5, @astrojs/starlight 0.35→0.41.7. Three breaking changes hit, all fixed: the Starlight 0.39 `autogenerate` sidebar shape (my plan wrongly said the sidebar was fully explicit — there IS one autogenerated group), Astro 7's deprecated `markdown.rehypePlugins` (migrated to `unified()` from `@astrojs/markdown-remark`), and a resolution failure where a **stray `/Users/jeetu/node_modules/cookie@0.5.0`** — outside the repo — shadowed astro's `cookie@2.0.1` and broke the build with "does not provide an export named `parseCookie`". Fixed in-repo with an explicit `cookie` dependency. The now-obsolete `vite.ssr.noExternal: ['nanoid']` workaround was removed after verifying the build is green without it. sharp 0.34.5 dropped out of the tree, so no sharp override was needed.
  - **Step 3 (4 alerts):** 33/34/35 dismissed `not_used`, alert 4 dismissed `tolerable_risk`. Two API constraints found: `dismissed_reason` must be one of five fixed values, and `dismissed_comment` caps at **280 characters** — the full evidence lives in `scripts/audit-allowlist.json` and here. PR #121 closed with a comment explaining why it could never merge alone (AC6).
  - **Step 4:** next-steps pruned + two follow-ups recorded; code-map row; ADR-0056 → accepted with an Outcome section; four lessons.
