...
- **Related**: ADR-0005 (playground stays Vite SPA), ADR-0013 (static hosting doctrine), ADR-0047 (desktop distribution; `releaseChannel.ts` single home), C4 (internal/ never ships), ADR-0048 (drafted by this task), lesson 2026-08-07 (derived documents: diff sets & modality)
...
8. **AC8 — C4 / leak safety.** Website content references nothing under `internal/`; the existing repo-wide leak scan passes over the new tree. (Checker guard from AC5 + existing scan.)
...
- `check-website-sync`: green on fresh manifest; **mutation checks** — edit a source → names the derived page, exit 1; delete a source → loud failure (not silent pass); manifest entry under `internal/` → refused. (Restore mutations by inverse edit, never `git checkout` — lesson 2026-08-21.)
