...
- Next step: owner restart + verify all five walk items; investigate doorbell delivery if hints still never land (safety net masks it now); then merge.
- Open questions: is the host pump's hint delivery actually reaching the frame on hardware? (Safety net covers the UX either way; verify before calling ADR-0034's push seam proven on Telepath.)
