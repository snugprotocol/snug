...
- **Status**: done — **both repos are PUBLIC as of 2026-08-26**; stage 8 executed and verified
...
- **Branch**: `chore/TASK-20260825-flip-public-execute` (PR #139) + `chore/…-readme-timing` (PR #140); the flip itself was a `gh` act on `main` by the owner
...
### 2026-08-26 — Claude + owner — session (THE FLIP)

- **Done — 🤖 A1–A3 (PR #139 `dca9e4b`, PR #140 `89b57e7`):** README footer repointed to `github.com/jeetumaker` (`jeetu.tech.voyage` proved **NXDOMAIN at the authoritative Cloudflare NS** — never created, not a host that was down) and the TechVoyage credit removed at the owner's ask; playground social card added (it had **zero** og/twitter meta) with an absolute `og:image` reusing the marketing origin's poster frame, pinned by a new 7-test `socialMeta.test.ts` over `dist/` and mutation-proven; the `~2 min` build+test claim corrected to **"under 3 min"** after measuring it cold at **137 s** (build 23 s + test 114 s). The org-landing pre-launch paragraph was staged uncommitted until the flip, then committed (`snugprotocol/.github@4a641c3`).
- **Done — 🔑 owner, all remaining stage-7 gates:** security@ round-trip verified · /terms counsel verified (**trademark deferred post-launch**) · walk ladder complete · **key custody closed** (both Desktop folders deleted, both secrets in 1Password with attachments verified; the Apple identity imported to the login keychain and proven by *actually signing* — full chain, no prompt, still signing with `~/Desktop/snug-csr/` moved aside).
- **Done — the flip.** Pre-flight sweep green on `89b57e7` (public-scrub OK · both purge SHAs fail to fetch + 422 · settings.local gitignored · **gitleaks 401 commits, no leaks** · root suite 25/25). Owner ran `gh repo edit` — **spec first**, then snug. Stage 8 executed and verified the same hour: private vulnerability reporting on both; **secret scanning + push protection enabled via `PATCH /repos/{r}` with `security_and_analysis`** (the runbook assumed Settings-UI-only — the API works once public); `main-pr-only` ruleset on both; **all 13 must-be-zero scrub terms return 0** in the public code search; anonymous reachability confirmed for repo pages, `/security/policy`, `/issues`, `/security/advisories/new`; 0.4 probe re-run post-flip still fails.
- **State:** `main` = **`bcf0738`**. Both repos **PUBLIC**. CI live and enforcing (see TASK-20260826-ci-restore). One secret-scanning alert open, **false positive** — a synthetic `sk-Ab3d…` fixture at `packages/protocol/src/__tests__/security.test.ts:44` that is the *input* proving the C1 guard detects OpenAI-shaped keys; left open deliberately so the dismissal carries the owner's name.
- **Next step:** owner uploads the two repo social previews (Settings → General; the control exists now), then the X/LinkedIn/Facebook re-scrape — **both before any announcement**, or links unfurl with GitHub's generic fallback.
- **Open questions:** dismiss alert #1 (owner or delegate to agent?) · one prefilled ADR-0052 feedback deep-link still needs an end-to-end run now that it no longer 404s · the org second-owner decision remains open (enforced 2FA + a single owner = a lost device is an unrecoverable org) · `apps/playground` OG tags are static; if it ever gains per-route SSR they should move into it.
