...
| Reference backend (/invoke SSE + model override, C1 boundary, artifact cache, Google OIDC + sessions/CSRF **+ `picture` claim**, /userdb CAS endpoints incl. revision-on-412, static hosting, fail-closed config, 30-min lifetimes + `step` progress events) | `apps/server/src` ✅ | `apps/server` vitest (104) |
| Playground (hub · builder · run+think-panel+versions · byok/local/subscription · user-DB state · sync/export/import · login) | `apps/playground/src` ✅ | vitest (190) + Playwright (30, incl. the C2 real-browser CSP gate + AC2 serverless gates + owner-report regressions) |
...
| **Two** `runAgentTurn` call sites — builder chat AND the app-frame transport; both must wire `onRoundTrip` or an app's LLM turns are invisible | `apps/playground/src/agent/{builder,transport}.ts` ✅ | playground `appTransportRoundTrips.test.ts` |
