...
| Provider adapters (anthropic/openai/mock), runAgentTurn choke point, http SSE transport, 48-iteration ceiling, partial-text-on-drop, 128K max output (8K local), `round_trip_start`+`round_trip`+timed tool events, wire model name, cache-token usage, **per-TURN** `cache` opt-in on `AdapterRequest` (never per-adapter — one instance serves turns of different shapes) | `packages/adapters/src` ✅ | `packages/adapters` vitest (90) |
