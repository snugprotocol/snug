...
- [0062 — A turn belongs to its thread, not to the view: navigation never aborts, only stop does](0062-thread-owned-turns-navigation-never-aborts.md) — **proposed** (TASK-20260903-build-thread-continuity): per-thread session store outlives every view; the unmount abort is removed; swap seams reset the registry; the hub create bar mints a fresh thread; parallel threads allowed.
