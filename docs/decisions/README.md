...
- [0066 — Tool-free brains carry the authoring knowledge in the prompt: one `knowledge` seat, three deliveries](0066-tool-free-knowledge-delivery.md) — **accepted** 2026-09-06 (TASK-20260906-tool-free-kb-inlining): `buildHostSystemPrompt({knowledge: 'tool' | 'inline' | 'none'})`, default byte-identical; `'inline'` = the 35 layer + the five-file core (10/20/30/40/80, 37,437 B, pinned by name in `layers.ts`) for the pinned host brain under `sample`'s 65,536-byte cap; `'none'` = the honest 36 layer for webllm (4,096-token window); the user slot's tool-free twin picked by `buildsToolFree`; a 44 KiB ceiling measured on the kit's own ruler; static selection, template whole, over-budget refuses by name.
- [0067 — The thinking level inside the host's brain is the user's (D15 amended, narrowly)](0067-host-brain-tier-control.md) — **accepted** 2026-09-06 (TASK-20260906-host-brain-tier-control): a `TierSeat` on the platform's host brain arm, the tier read at call time, the control in the brain chip's popover only where a seat exists, substitution disclosed from `modelTierApplied`; Q1–Q4 (scope, per-purpose default, persistence rung, substituted options) to the owner.
- [0068 — Binding B is a plugin-bundled local process: the desktop's native side, in Node, on loopback](0068-local-host-process.md) — **accepted** 2026-09-07 (TASK-20260907-binding-b-plugin-host, re-scopes T3 of the skill-only program): `apps/host-mcp` spawned by the host over stdio; serves `snug-host-local.html`; fills `fetchImpl`/`oauth`/`userdbBackend` on loopback with the executor's gates re-run; four control-plane tools and no data-plane tool; bearer in the fragment, never in an MCP message; one process per machine over a `0600` control socket; Q1–Q5 to the owner.
- [0069 — Bindings and brains are two axes; MCP is Binding B's spawn channel, never a binding and never a brain](0069-bindings-and-brains-two-axes.md) — **accepted** 2026-09-13 (TASK-20260913-binding-b-marketplace-plugin): the owner's "path 2" IS the merged Binding B — a binding is where the body runs, a brain is what answers the thinks (here the user's own CLI as a child process); the process and its artefacts are renamed `local-host` before the first publish; "artifact + local proxy" (path 1) is recorded as the later Binding A-connected phase with S12 beside it; the brain keeps a warm child per system prompt over stream-json (measured: ~1.3 s follow-up turns against ~5 s cold), streams deltas, names `outdated`, and finds its binary off an empty GUI PATH; the plugin is the distribution repo's root with the skill generated from the prompt store into a gitignored tree; the community marketplace is the target.
