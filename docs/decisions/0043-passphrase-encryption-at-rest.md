...
3. **A Recovery Key is mandatory, not optional.** ≥128 bits (26 symbols from a 30-glyph alphabet with `0/O/1/l/I` excluded, rejection-sampled). `encryptContainer` **refuses** to build a single-slot container. This is what makes ADR-0014's named cost payable: without a second independent way in, this feature ships a data-loss mode with no mitigation.
