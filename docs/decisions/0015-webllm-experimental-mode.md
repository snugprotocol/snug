...
- Real-model e2e cannot run in CI (download + WebGPU): the Playwright spec exists but is skip-by-default behind `SNUG_E2E_WEBLLM_REAL=1`; contract coverage rides the faked-engine unit suite (45 tests) and the WebGPU-absent fallback e2e.
