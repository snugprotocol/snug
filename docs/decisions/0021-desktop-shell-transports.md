...
2. **Loopback callback transport (desktop default).** `tauri-plugin-oauth` listener bound to `127.0.0.1` only; redirect URI `http://127.0.0.1:{port}/callback` — the IP literal, never `localhost` (Spotify bans it; RFC 8252 prefers it). `loopback-fixed-port` entries use the fixed desktop port **41420**; a bind collision is an honest wizard error, never a silent fallback port (a fallback would break the URI the user registered). Ephemeral ports only for `loopback` entries whose provider honors any-port loopback. The flow logic is untouched: HMAC-signed state, caller-held `expectedFlowId`, nonce single-use, PKCE S256 — a forged/replayed hit on the local listener delivers nothing that survives `handleCallback`'s guards, and the listener's response page carries zero token material.
