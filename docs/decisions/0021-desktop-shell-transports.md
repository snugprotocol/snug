...
6. **`.snug` file association.** Desktop registers `.snug` (same sqlite byte format; a filename convention, not a new format). — **COMPLETED 2026-08-20 by [ADR-0042](0042-snug-file-extension.md)**: `.snug` became the canonical name on every platform, not just the desktop association. Note the byte-format clause no longer holds unconditionally — [ADR-0043](0043-passphrase-encryption-at-rest.md) makes a `.snug` optionally an encrypted container. Opened files (macOS `RunEvent::Opened`; Windows/Linux argv via single-instance) route through an explicit user confirmation into `importUserFile`, arming the F15 endpoint-confirm guard. No silent import.
