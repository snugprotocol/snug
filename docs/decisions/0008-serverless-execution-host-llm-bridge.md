...
- BYOK keys live in the user DB (portable) — at-rest posture documented honestly (no KMS claim until KeyProvider ships, per ADR-0003's honesty rule). To keep "never sent to any hub server" true under ADR-0009 sync, the secrets table is **stripped from hub-origin sync and default export**; including secrets is an explicit user opt-in (personal origins / full-portability export).
