...
- **C2 — Sandbox integrity.** Runner iframes: `sandbox="allow-scripts"` only (never `allow-same-origin`), CSP with `connect-src 'none'` for app-originated traffic, fixed CDN allowlist (jsdelivr, cdnjs, tailwind). Any change to these attributes is High-tier and needs an ADR.
