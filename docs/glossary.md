...
- **Artifact record** — the hosted artifact's durable-copy seam (`apps/host/src/storage/artifactHtml.ts`): the browser bucket stays the WORKING copy; the page's own `snug-db` block is the DURABLE copy, written by one explicit act that fetches the page's canonical source, verifies it with the shared tokenizer, splices the file in as base64 and republishes. Seed-on-empty, divergence with a direction (the save counter), refusals by name, never a timer.
