...
- **Brain** — what answers the apps' thinks, the second axis beside the binding (ADR-0069): the viewer-billed `sample` / `window.claude.complete` (Binding A only), **the user's own agent CLI as a child process** (Binding B — `claude` today, through the local host process's pre-warmed single-use children; `codex exec` / Hermes / OpenClaw / Ollama later), or the demo brain (the fallback everywhere). Never MCP: it is B's spawn channel, not a brain.
