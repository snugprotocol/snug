...
- **Probe** — the host kit's boot step, before React (`apps/host/src/probe.ts`): the **binding** decided purely from four facts (protocol, hostname, whether `window.claude.use` / `window.claude.complete` are functions), the **storage** TRIED rung by rung with a real write/read round trip (OPFS → IndexedDB → memory — never presence-detected: `file://` exposes OPFS and rejects it), and the **brain** legs recorded (`sample` / `complete` detected or absent; the demo brain pinned until T3/T4 wire a leg). Nothing asks the user anything (D15).
