...
| **S3** | **A bundle carries DDL** (`schema.ddl[]`) that `applyAppDdl` executes | Smuggle rows (`CREATE TABLE … AS SELECT`) or a second statement (a trigger sandwich) under "structure only"; run DDL before install | The schema admits only single `CREATE …` statements (no CTAS, no virtual tables, one `BEGIN` per trigger and no DDL keyword after a `;`, comment tokens refused) — and the driver's own `MULTI_STATEMENT` refusal is the enforcement of last resort; DDL runs ONLY inside the install and update acts, against the app's OWN runtime database; an install failure removes the half-made app, and an update runs DDL BEFORE landing the new html so a failure leaves the old code current |
| **S4** | **A bundle carries a runtime contract** — third-party text for an LLM system slot, which ADR-0018 D3 forbids on untrusted channels | Steer the recipient's model inside the app; reach anything the host layers guard; slip a contract in through an UPDATE with no review moment | Admitted ONLY through the preview-then-install / preview-then-update act (owner Q6b, ADR-0063 §8): the preview shows EVERY field the assembly renders — overview, persona, state and response guidance, the settings lines, the output cap — as text before install, and an update of an installed lineage is reachable only from that same preview; `importUserDb`'s drop rule is untouched for every other channel. The host puts no secret in any LLM context (C1), so the blast radius is the app's own frame |
| **S12** | **A lineage is an unauthenticated identity** — `lineage` is in every copy of a bundle, and an installed copy is matched on `share:<lineage>` alone | Forge a bundle with a known lineage to plant an "update" offer that hands an installed app — and every grant it holds — to attacker code | The update act runs ONLY from the preview (docs + contract readable) and ALWAYS confirms, naming the approved connections the new code inherits and stating that anyone who knows the lineage can offer an update; the installed app's header is a door to that preview, never a write; a link visit never persists the offer. Bundle signing is a stated non-goal at v1 (R-i) |
| **S5** | **A received bundle before install** | Write third-party bytes into the user file on a bare link visit; fill the shelf to evict what the user has not seen; render markup from the bundle | Memory-first: a link visit persists nothing (`sharedApp:` rows are written only for an opened attachment; a link preview installs straight from memory — the "keep" step was removed 2026-09-04, TASK-20260904-share-link-ux); the cap REFUSES the 13th with a note, never evicts; every bundle string renders as a text node (hostile-bundle test) |
...
### R-c — A blind blob drop is a generic anonymous file host for up to 30 days
The relay cannot validate content it cannot read; anyone can store ≤ ~1 MiB of ciphertext
per upload, for the lifetime the uploader chooses from a closed set (1 / 7 / 30 days, default
7 — `TTL_DAYS` is the ceiling and a choice above it is refused). Mitigated by the cap, the
TTL, a per-IP rate-limiting rule (`deploy-relay.mjs ratelimit`, clamped to the zone's plan;
runbook), and the fact that far better free hosts exist. Turnstile is queued if abuse
appears.
...
### R-i — An update offer is as trustworthy as the lineage, which is public
Anyone who has ever received a bundle of an app knows its `lineage` and can craft a
"newer version" for it. The recipient sees it as an update to an app they trust, and an
approved grant survives the update (ADR-0045's inheritance, by design). Mitigated by
the preview-only update path, the always-on confirm that names the inherited grants,
and the recipient being the reviewer; NOT mitigated by any authentication of the
sharer — bundle signing is a stated non-goal at v1. A follow-up worth its own ADR:
demote an installed copy's approved rows to `declared` when a shared update changes the
html (a sixth `snug_connections` writer — a spec change, hence not done here).
