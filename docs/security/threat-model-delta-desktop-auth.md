...
| **S7** | **Auth-shaped failure observer** | Use the observer as an oracle, or leak through it | Host-only callback carrying `(appId, slot, status)` plus, since TASK-20260815 AC4 (ADR-0028 era), an optional `detail`: a ≤160-char plain-text extract of the provider's error reason, read from the **gate-10-scrubbed, size-capped delivered body only** — never the raw `Response`, so every injected-credential form (raw and percent-encoded) is already scrubbed before extraction, pinned by a query-credential echo test. No credentials, no URLs, no raw response bytes; recognized JSON error shapes or a text head only (HTML and unrecognized JSON yield nothing). Rendered as plain text — markup never becomes elements, URLs never become links (hostile-copy test, same rule as registration steps). TASK-20260819 moved that render from the run-surface banner to the wizard's attention gate (`ConnectionWizardSheet`); the hostile-copy test moved WITH it rather than being dropped, so the no-markup/no-linkify property is pinned at the new render site. The run-surface chip does not render `detail` at all, which narrows this surface rather than widening it. Fires only on the **final delivered result** of `execute()`, so a 401 cured by the OAuth refresh retry fires nothing; `executeConnectionTestRequest` strips the seat entirely so wizard probes never fire it. The app-visible result is unchanged (`ok:true`, status as-is) — the app contract is not broken to gain visibility. Clean bill issued at the P0 security lens; re-reviewed at the TASK-20260815 plan review (the three "no response bytes" doctrine comments were rewritten with the code in one commit). |
...
decision**~~ — **RESOLVED 2026-08-20: the shell ships macOS-only through alpha, beta and
1.0; Windows desktop is reconsidered post-1.0** (ADR-0021 D8
