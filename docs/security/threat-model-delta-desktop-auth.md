...
| **S7** | **Auth-shaped failure observer** | Use the observer as an oracle, or leak through it | Host-only callback carrying `(appId, slot, status)` and nothing else — no credentials, no response bodies. Fires only on the **final delivered result** of `execute()`, so a 401 cured by the OAuth refresh retry fires nothing; `executeConnectionTestRequest` strips the seat entirely so wizard probes never fire it. The app-visible result is unchanged (`ok:true`, status as-is) — the app contract is not broken to gain visibility. Clean bill issued at the P0 security lens and unchanged since. |
