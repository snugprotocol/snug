// Vite config for the LOCAL page (ADR-0068 D-B1) — the second build of `apps/host`.
//
// A second INPUT in the kit's own config is impossible: `inlineDynamicImports` refuses
// multiple inputs, `inline-single-file` throws on more than one page or entry chunk, and
// `check-host-kit` requires `dist/` to hold exactly `snug-host.html`. So this is a second
// CONFIG writing to its own `dist-local/`, which the kit's gate never sees — the pattern
// the kit's own header names for the micro kit.
//
// Everything below is the kit's config with three changes: the entry (`local.html`), the
// output name, and the output directory. The plugins, aliases, inlining rules and refusals
// are identical BY CONSTRUCTION rather than by restatement.
//
// ---- the kit config's own notes follow ----
...
export default defineConfig({
  plugins: [
    ...
    react(),
    inlineSingleFile({ stamp, fileName: 'snug-host-local.html' }),
  ],
  resolve: {
    alias: {
  ...
  assetsInclude: ['**/*.wasm'],
  build: {
    // Its own directory: `check-host-kit` scans `apps/host/dist` and requires exactly one
    // file there, so the local page must never land beside the artifact kit.
    outDir: 'dist-local',
    rollupOptions: { input: here('./local.html'), output: { inlineDynamicImports: true } },
    target: 'es2022',
