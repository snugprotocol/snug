// COPIED from `apps/whatsapp-sidecar/src/parent-watch.ts`, deliberately (D-B22).
//
// Importing it across the app boundary does not work: that package is private, declares no
// `exports` map, sets `rootDir: src`, and its barrel re-export reaches `baileys` — which
// `ssr.noExternal` would inline into this bundle, contradicting the dependency claim and
// failing the release-inertness sweep. So the 80 lines ride here with an EQUIVALENCE TEST
// against the original (`parent-watch.test.ts`): one behaviour, two files, byte-compared,
// which is the repo's own remedy when a shared source cannot be shared.
//
// ---- the original's own header follows ----
