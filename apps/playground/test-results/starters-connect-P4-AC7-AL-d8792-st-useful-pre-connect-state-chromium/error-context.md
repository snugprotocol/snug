...
- Name: starters-connect.spec.ts >> P4-AC7 / AL-09 AC8 — the degraded pre-connect state is real >> weather boots into an honest, useful pre-connect state
...
Locator: locator('[data-testid="frame-wrap"] iframe[sandbox="allow-scripts"]').contentFrame().getByRole('heading', { name: /connect openweather/i })
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="frame-wrap"] iframe[sandbox="allow-scripts"]').contentFrame().getByRole('heading', { name: /connect openweather/i })
