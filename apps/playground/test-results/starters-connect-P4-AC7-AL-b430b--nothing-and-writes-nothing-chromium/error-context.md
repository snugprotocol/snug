...
- Name: starters-connect.spec.ts >> P4-AC7 / AL-09 AC8 — the degraded pre-connect state is real >> a READ-ONLY starter route reaches nothing and writes nothing
- Location: e2e/starters-connect.spec.ts:167:3
...
Locator: locator('[data-testid="frame-wrap"] iframe[sandbox="allow-scripts"]').contentFrame().getByLabel(/search for a city/i)
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="frame-wrap"] iframe[sandbox="allow-scripts"]').contentFrame().getByLabel(/search for a city/i)
...
- main:
  - text: "Should I? Forecasts turned into decisions: good/maybe/skip verdicts for the plans you actually have, a comfort timeline, saved places, and a decision journal you own."
  - button "install"
  ...
  - strong: OpenWeather
  - text: (api.openweathermap.org). installing only copies the app — nothing is connected until you review and approve it yourself.
  - iframe
...
> 173 |     await expect(app.getByLabel(/search for a city/i)).toBeVisible({ timeout: 20_000 });
  ...
  264 |      * once a desktop e2e project exists to carry it.
  265 |      */
  266 |     await page.goto('/');
  267 |     const tile = page.locator('[data-testid="starter-tile"][data-starter-name="trade copilot"]');
  268 |     await expect(tile).toBeVisible({ timeout: 20_000 });
  269 |     await expect(tile.getByTestId('desktop-only-badge')).toBeVisible();
  270 |     await expect(tile.getByTestId('desktop-only-badge')).toContainText(/desktop app/i);
  271 |     await expect(tile.locator('.tile-card-button')).toBeDisabled();
  272 |     await expect(tile.getByTestId('starter-install')).toHaveCount(0);
  273 |   });
```